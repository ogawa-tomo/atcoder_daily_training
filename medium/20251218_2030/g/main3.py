...
class CookiesManager:
    def __init__(self, cookies: list[list[Cookie]]):
        ...
        # self.existing_rows: set[int] = set()
        # for i in range(H):
        #     self.existing_rows.add(i)
        # self.existing_columns: set[int] = set()
        # for j in range(W):
        #     self.existing_columns.add(j)

        self.row_exists: list[bool] = [True] * H
        self.column_exists: list[bool] = [True] * W
        self.row_num = H
        self.column_num = W
    ...
    # def row_num(self):
    #     return len(self.existing_rows)

    # def column_num(self):
    #     return len(self.existing_columns)

    def row_meets_condition(self, i: int) -> str:
        # if i not in self.existing_rows:
        #     return ""
    ...
    def column_meets_condition(self, j: int) -> str:
        # if j not in self.existing_columns:
        #     return ""
        if not self.column_exists[j]:
            return ""
        if self.row_num <= 1:
            return ""
        for s in alphabets:
    ...
    def delete_row(self, i: int, s: str):
        ...
        # self.existing_rows.remove(i)
        self.row_exists[i] = False
        self.row_num -= 1

    def delete_column(self, j: int, s: str):
        self.deleted_count += self.row_num
        self.deleted_column_num[s] += 1
        # self.existing_columns.remove(j)
        self.column_exists[j] = False
        self.column_num -= 1
